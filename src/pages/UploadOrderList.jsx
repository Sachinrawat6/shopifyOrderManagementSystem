...
import Papa from "papaparse";
import axios from "axios";
import { FiUpload, FiCheckCircle, FiClock, FiSend, FiLoader, FiCrop, FiX } from "react-icons/fi";
  ...
  const handleFileUpload = (e) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        let filteredData = finalData.filter((d) => d["Financial Status"]?.toLowerCase() !== "voided");
        let voidedOrders = finalData.filter((d) => d["Financial Status"]?.toLowerCase() === "voided")?.length;
        setData(filteredData);
        setVoided(voidedOrders)
      },
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {error && (
          <div className={`p-4 mb-6 rounded-lg ${error.startsWith("Success") || error.includes("Processed")
            ? "bg-green-100 text-green-800 border-l-4 border-green-500"
            : "bg-red-100 text-red-800 border-l-4 border-red-500"
            }`}>
            {error.split('\n').map((line, i) => (
        ...
        {data.length > 0 && (
          <>
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-700">
                  Order Summary ({(data.length || 0) + (voided || 0)}  total)
                </h2>
                <button
                  onClick={sendOrdersToBackend}
                  disabled={sending}
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-red-50 p-4 rounded-lg flex flex-col justify-center items-center  border border-red-200">
                  <div className="flex items-center">
                    ...
                    <span className="font-medium">PreCancel Orders</span>
                  </div>
                ...
                <div className="bg-red-400 p-4 flex flex-col justify-center items-center rounded-lg border border-red-200">
                  <div className="flex items-center">
                    ...
                    <span className="font-medium text-white">Voided Orders</span>
                  </div>
                  <p className="text-2xl font-bold mt-2 border-2 border-white w-12 h-12 grid items-center text-center rounded-full" >
                    {voided}
                  </p>
                    ...
                    {data.map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs rounded-full ${isConfirmed
                              ? "bg-green-100 text-green-800"
                              : "bg-yellow-100 text-yellow-800"
                              }`}>
                              {isConfirmed ? "Confirmed" : "Pending"}
